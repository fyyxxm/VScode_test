...
{
	decaIrqStatus_t s = NVIC_GetEnableIRQ(EXTI9_5_IRQn); 

	if(s) {
		    NVIC_DisableIRQ(EXTI9_5_IRQn); //disable the external interrupt line
	}
...
)
{
    ...
    // Write it to the SPI
    SPI1_WriteToDW(cnt,header,length,buffer);
...
)
{
    ...
    SPI1_ReadFromDM(cnt, header, length, buffer);  // result is stored in the buffer
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int8_t _dwt_otpsetmrregs(int mode)
{
    uint8 wr_buf[4];
